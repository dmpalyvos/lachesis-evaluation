...
./scripts/run.py ./scripts/templates/FlinkLinearRoadKafka.yaml -d "$DURATION" -r "$REPS" --statisticsHost "$(hostname)" --kafkaHost "$KAFKA_HOST" -c "$DATE_CODE" --sampleLatency true
