...
class Score extends BaseRichBolt {
    ...
    public void declareOutputFields(OutputFieldsDeclarer outputFieldsDeclarer) {
        outputFieldsDeclarer.declare(new Fields("timestamp", "timestamp_ext", "calling_number", "answer_timestamp", "score", "cdr"));
    }
    ...
    public void execute(Tuple tuple) {
        ...
        if (map.containsKey(key)) {
            ...
            if (entry.isFull()) {
                ...
                Values out = new Values(timestamp, timestamp_ext, number, answerTimestamp, mainScore, cdr);
                outputCollector.emit(out);
                LOG.debug("tuple out: {}", out);
            } else {
                entry.set(source, score);

                Values out = new Values(timestamp, timestamp_ext, number, answerTimestamp, 0.0, cdr);
                outputCollector.emit(out);
                LOG.debug("tuple out: {}", out);
            }
        } else {
