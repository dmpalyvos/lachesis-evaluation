...
class PreRCR extends BaseRichBolt {
    ...
    public void declareOutputFields(OutputFieldsDeclarer outputFieldsDeclarer) {
        outputFieldsDeclarer.declare(new Fields("key", "timestamp", "timestamp_ext", "calling_number", "called_number", "answer_timestamp", "new_callee", "cdr", "ecr"));
    }
    ...
    public void execute(Tuple tuple) {
        ...
        Values values = new Values(callingNumber, timestamp, timestamp_ext, callingNumber, calledNumber, answerTimestamp, newCallee, cdr, ecr);
        outputCollector.emit(values);
        LOG.debug("tuple out: {}", values);

        values = new Values(calledNumber, timestamp, timestamp_ext, callingNumber, calledNumber, answerTimestamp, newCallee, cdr, ecr);
        outputCollector.emit(values);
        LOG.debug("tuple out: {}", values);
