...
public class ECR_Reordering extends BaseRichBolt {
    ...
    public void declareOutputFields(OutputFieldsDeclarer outputFieldsDeclarer) {
        outputFieldsDeclarer.declare(new Fields("timestamp", "timestamp_ext", "calling_number", "called_number", "answer_timestamp", "new_callee", "cdr", "ecr"));
    }
    ...
    public void execute(Tuple tuple) {
        ...
        if (cdr.callEstablished) {
            ...
            Values out = new Values(timestamp, timestamp_ext, cdr.callingNumber, cdr.calledNumber, cdr.answerTimestamp, newCallee, cdr, ecr);
            outputCollector.emit(out);
            LOG.debug("tuple out: {}", out);
        } else {
            Values out = new Values(timestamp, timestamp_ext, cdr.callingNumber, cdr.calledNumber, cdr.answerTimestamp, newCallee, cdr, -1.0);
            outputCollector.emit(out);
            LOG.debug("tuple out: {}", out);
        }
