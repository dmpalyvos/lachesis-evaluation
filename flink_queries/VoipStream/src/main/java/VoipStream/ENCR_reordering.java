...
public class ENCR_reordering extends RichFlatMapFunction<
    ...
    public void flatMap(Tuple8<Long, String, String, Long, Boolean, CallDetailRecord, Double, Long> tuple, Collector<Tuple8<Integer, Long, String, Long, Double, CallDetailRecord, Double, Long>> collector) {
        ...
        if (cdr.callEstablished) {
            ...
            if (tuple.f6 >= 0) {
                Tuple8<Integer, Long, String, Long, Double, CallDetailRecord, Double, Long> out = new Tuple8<>(ScorerMap.ECR, timestamp, caller, cdr.answerTimestamp, tuple.f6, cdr, -1.0, timestamp_ext);
            ...
            if (newCallee) {
                ...
                Tuple8<Integer, Long, String, Long, Double, CallDetailRecord, Double, Long> out = new Tuple8<>(ScorerMap.ENCR, timestamp, caller, cdr.answerTimestamp, rate, cdr, -1.0, timestamp_ext);
                collector.collect(out);
                LOG.debug("tuple out: {}", out);
            }
        }
