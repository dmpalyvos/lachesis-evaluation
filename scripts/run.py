...
def stop_active_processes(sig):
    for process in ACTIVE_PROCESSES:
        try:
            ...
            logging.error(f'Exception when terminating process {process.args[0]} ({process.pid}): {e}')
...
def verify_experiment_config(config):
    if has_duplicates([execution['name'] for execution in config['queries']]):
        raise AssertionError(f'Duplicate query name')
...
def get_parser():
    ...
    parser.add_argument('--duration', '-d', help='Experiment duration',
                        type=float,
    ...
    parser.add_argument('--statisticsHost', help='Host for DropWizard metrics', required=True)
    parser.add_argument('--kafkaHost', help='Host of kafka remote source', type=str, default='')
    ...
    parser.add_argument('--code', '-c', type=str, help='Use custom code instead of day_time',
                        default=None)
    parser.add_argument('--reps', '-r', help='Number of repetitions', type=int, default=1)
    parser.add_argument('--dry', help='Dry run, just print commands and create dirs',dest='dry', action='store_true')
...
if __name__ == '__main__':
    ...
    time_code = datetime.datetime.now().strftime('%j_%H%M') if not args.code else args.code
    experiment_code = f'{commit_hash}_{time_code}' if not args.resume else args.resume
    jar = config['jar'].format(commit_hash=commit_hash)

    output_folder = config['output_folder'].format(experiment_code=experiment_code)
    if args.resume:
    ...
    try:
        ...
        if not args.resume:
            print(f'Warning: Directory {output_folder} already exists! Proceeding anyway in 3 seconds...')
            time.sleep(3)
    ...
    execute_quick(f'python3 {COMPILE_SCRIPT} {args.experiment} {output_folder}/{EXPERIMENT_CONFIG_NAME}')
    ...
    # Compute total experiment queries
    total_runs = args.reps * len(experiment_config['queries']) * len(variants)

    query_names = [e['name'] for e in experiment_config['queries']]
    variant_names = [v['name'] for v in variants]
    total_duration = total_runs * args.duration
    logging.info('-'*100)
    logging.info(f'> Repetitions: {args.reps}')
    logging.info('> Queries: %s', '.'.join(query_names))
    logging.info('> Variants: %s', ','.join(variant_names))
    logging.info(f'> Total Duration = {total_duration} minutes ({total_duration/60:.1f} hours)')
    logging.info('-'*100)
    time.sleep(5)
    ...
    for rep in range(1, args.reps+1):
        for query in experiment_config['queries']:
            # Shuffle variants
            random.shuffle(variants)
            for variant in variants:
                run_name = f'{query["name"]}_{variant["name"]}'
                statistics_folder = f'{base_dir}/{output_folder}/{run_name}/{rep}'
                cpu_utilization_file = statistics_folder + os.sep + config['cpu_utilization_file']
                logging.info('-'*100)
                logging.info(f'Executing {run_name} / rep {rep}')
                spe_command = get_command(variant['spe_command'], variant, experiment_config, jar=jar, jvm_args=jvm_args, 
                                            class_name=query['reference'], statistics_folder=statistics_folder, duration_seconds=duration_seconds,
                                          rep=rep, statistics_host=args.statisticsHost, 
                                          extra_args=extra_args, random_seed=1, kafka_host=args.kafkaHost)
                utilization_command = get_command(utilization_command_template, variant, experiment_config, cpu_utilization_file=cpu_utilization_file, statistics_host=args.statisticsHost)
                scheduler_command = get_optional_command('scheduler_command', variant, experiment_config, statistics_folder=statistics_folder, statistics_host=args.statisticsHost)
                kafka_start_command = get_optional_command('kafka_start_command', variant, experiment_config, statistics_host=args.statisticsHost, kafka_host=args.kafkaHost)
                kafka_stop_command = get_optional_command('kafka_stop_command',variant, experiment_config, statistics_host=args.statisticsHost, kafka_host=args.kafkaHost)

                if args.dry:
                    print(spe_command)
                    print(kafka_start_command)
                    print(kafka_stop_command)
                ...
                for retry in range(MAX_RETRIES):
                    ...
                    execution_config_path = create_execution_config(
                        statistics_folder, spe_command=spe_command, utilization_command=utilization_command, scheduler_command=scheduler_command,
                        kafka_start_command=kafka_start_command, kafka_stop_command=kafka_stop_command,
                        duration_seconds=duration_seconds, statistics_folder=statistics_folder, statistics_host=args.statisticsHost, experiment_yaml=f'{output_folder}/{EXPERIMENT_CONFIG_NAME}', job_name=job_name)
                    experiment_process = execute(f'{executor_script} {execution_config_path}')
                    monitor(experiment_process)

                    try:
                      exit_code = experiment_process.wait(duration_seconds+60)
                    except subprocess.TimeoutExpired:
                      logging.warning('Experiment timed out. Forcing exit')
                      stop_active_processes(signal.SIGTERM)
                      try:
                        exit_code = experiment_process.wait(180)
                      except subprocess.TimeoutExpired:
                        logging.error(
